...
def main(args):
    ...
    ref = Referee(args.columns, args.rows, args.c, args.p, blue_player,
...
def positive_integer(value):
    ...
    if ivalue <= 0:
        raise argparse.ArgumentTypeError(
            '%s Valor inválido, tiene que ser un numero natural.' % value)
    return ivalue


if __name__ == '__main__':

    description = 'Versión del laboratorio de AED III del juego cuatro en' \
                  ' linea para el TP3.'
    ...
    parser.add_argument('--c',
                        type=positive_integer,
                        default=4,
                        help='El tamaño de la linea a formar. c <= mínimo'
                             ' {columns, rows}.')
    ...
    parser.add_argument('--first',
                        type=str,
                        choices=[BLUE, RED],
                        help='Color del jugador que comienza, si no está'
                             ' especificado comienza siempre un jugador random'
    ...
    parser.add_argument('--ui',
                        type=bool,
                        default=False,
                        help='Si es True, la aplicacion muestra una interfaz'
                             ' grafica.')
